...
export const updateRemotePlaybackClock = (snapshot: RemotePlaybackSnapshot) => { setAnchor({ ...snapshot, at: performance.now() }); };
...
export async function commandRemotePlayback(command: RemotePlaybackCommand, value?: number): Promise<void> {
    ...
    await enqueue(async () => {
        ...
        setAnchor({ ...anchor, position, at: performance.now(), playing: command === 'pause' ? false : command === 'play' ? true : anchor.playing });
    });
}
