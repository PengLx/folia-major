...
export const usesMusicKit = () => Boolean(typeof window !== 'undefined' && window.electron?.appleMusicRequest)
    && readProviderSessionValue('applemusic', 'mode') !== 'cider';

export function setAppleMusicMode(mode: 'musickit' | 'cider') {
    writeProviderSessionValue('applemusic', 'mode', mode);
}
