...
- `appleMusicProvider.ts`：Apple Music adapter；`appleMusic/transport.ts` 通过 `electron/appleMusic/host.cjs` 访问隔离 MusicKit 会话，`appleMusic/normalize.ts` 保留 catalog/library 身份。完整音频不暴露 URL；普通调用通过 Omni 的 remote playback 方法和 `src/services/remotePlayback.ts` 管理播放所有权与时钟，不进入音频缓存和 Automix。
