...
import { applePath, getStorefront, hasCiderBridge, requestApple } from './appleMusic/transport';
import { removeProviderSessionValue, writeProviderSessionValue } from './providerStorage';
import { ciderPlayback } from './appleMusic/playback';
import { requestMusicKit, setAppleMusicMode, usesMusicKit } from './appleMusic/musicKitTransport';
import {
...
export const appleMusicProvider: OnlineMusicProvider = {
    ...
    getAvailability: () => ({ configured: hasCiderBridge(), reason: hasCiderBridge() ? undefined : 'runtime-unavailable' }),
    // Reads work in both modes; ratings and playlist writes need the standalone player because
    // Cider's loopback API only relays GET requests.
    get capabilities(): ProviderCapabilities {
        const writable = usesMusicKit();
        return { search: true, playback: true, lyrics: true, wordByWordLyrics: true, auth: true,
            userLibrary: true, playlists: true, albums: true, artists: true, userAlbums: true, userCloud: true, recommendations: true,
            mutations: writable, likes: writable, playlistTrackMutations: writable, playlistSubscription: writable };
    },
    ...
    auth: {
        async configureConnection({ applicationToken, developerToken, playbackMode }) {
            if (playbackMode) setAppleMusicMode(playbackMode);
            if (usesMusicKit()) {
                await requestMusicKit('connect', { developerToken });
                removeProviderSessionValue('applemusic', 'disconnected');
                return;
            }
            if (!window.electron?.ciderConfigure) throw new OnlineProviderError('unavailable', 'Desktop required.', 'applemusic');
            if (applicationToken !== undefined) await window.electron.ciderConfigure(applicationToken);
            removeProviderSessionValue('applemusic', 'disconnected');
        },
        ...
        async logout() {
            if (usesMusicKit()) await requestMusicKit('logout');
            writeProviderSessionValue('applemusic', 'disconnected', 'true');
            await window.electron?.ciderConfigure('');
        },
    },
    library: {
