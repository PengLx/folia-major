...
describe('Apple Music favourites and playlist writes', () => {
    ...
    it('exposes writes only in standalone mode because Cider relays reads', async () => {
        const cider = vi.fn(async () => ({ status: 200, data: { data: { data: [] } } }));
        vi.stubGlobal('window', { electron: { ciderRequest: cider } });
        const capabilities = omni.getProviderCapabilities('applemusic');
        expect(capabilities).toMatchObject({ likes: false, mutations: false, albums: true, recommendations: true });
        await expect(omni.likeSong(normalizeAppleSong(song), true)).rejects.toMatchObject({ code: 'unsupported' });
        expect(cider).not.toHaveBeenCalled();
    });
});
