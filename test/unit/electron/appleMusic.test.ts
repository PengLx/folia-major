...
const { validateDeveloperToken, createCredentials } = require('../../../electron/appleMusic/credentials.cjs');
...
describe('isolated Apple Music host boundary', () => {
    ...
    it('refuses plaintext Linux storage and encrypts the saved JWT', () => {
        const data = new Map();
        const store = { get: (key: string) => data.get(key), set: (key: string, value: string) => data.set(key, value) };
        const safe = { isEncryptionAvailable: () => true, getSelectedStorageBackend: () => 'basic_text', encryptString: () => Buffer.from('ciphertext') };
        const jwt = token(Date.now() / 1000 + 3600);
        expect(() => createCredentials(store, safe).set(jwt)).toThrow('credential-storage-unavailable');
        safe.getSelectedStorageBackend = () => 'gnome_libsecret';
        createCredentials(store, safe).set(jwt);
        expect(data.get('appleMusic.developerToken')).toBe(Buffer.from('ciphertext').toString('base64'));
