...
async function prepare(page: import('@playwright/test').Page) {
    ...
    await page.addInitScript(() => {
        ...
        Object.assign(window.electron!, {
            ...
            ciderConfigure: async () => true,
            ciderRequest: async (request: { path: string; body?: Record<string, unknown> }) => {
                commands.push(request);
                if (request.path.endsWith('/run-v3')) {
                    const path = String(request.body?.path);
                    if (path.includes('storefront')) return { status: 200, data: { data: { data: [{ id: 'us' }] } } };
                    if (path.includes('/search?')) return { status: 200, data: { data: { results: { songs: { data: [{ id: '42', type: 'songs', attributes: { name: 'Remote song', artistName: 'Artist', albumName: 'Album', durationInMillis: 120000 } }] } } } } };
                    if (path.includes('playlists')) return { status: 200, data: { data: { data: [] } } };
                    return { status: 200, data: { data: { data: [] } } };
                }
                if (request.path.endsWith('/is-playing')) return { status: 200, data: { is_playing: playing } };
                if (request.path.endsWith('/now-playing')) return { status: 200, data: { info: { playParams: { id: '42' }, currentPlaybackTime: position, durationInMillis: 120000 } } };
                if (request.path.endsWith('/pause')) playing = false;
                if (request.path.endsWith('/play')) playing = true;
                if (request.path.endsWith('/seek')) position = Number(request.body?.position);
...
test('uses the remote clock, pause/resume and seek while the audio elements remain empty', async ({ page }) => {
    ...
    await expect.poll(() => page.evaluate(() => (window as any).appleTestCommands.some((r: any) => r.path.endsWith('/seek')))).toBe(true);
    await page.evaluate(async () => {
        const path = '/src/stores/usePlaybackStore.ts';
...
test('searches the selected Apple Music provider and starts its result from the UI', async ({ page }) => {
    ...
    await expect.poll(() => page.evaluate(() => (window as any).appleTestCommands.some((r: any) => r.path.endsWith('/play-item') && r.body?.id === '42'))).toBe(true);
    await expect.poll(() => page.evaluate(async () => {
        const path = '/src/stores/usePlaybackStore.ts';
...
test('opens Apple authorization directly from the account menu without a Folia confirmation dialog', async ({ page }) => {
    ...
    await expect(page.getByText('Cider', { exact: true })).toHaveCount(0);
