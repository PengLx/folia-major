...
已实现搜索、个人歌单、catalog/library 播放、订阅检查、暂停/恢复、音量、定位、播放时钟与自动续播，以及 TTML 逐字歌词。数据通过 `appleMusicProvider.ts` 和 `appleMusic/transport.ts` 归一化后进入 Omni；播放通过 `remotePlayback.ts`，不存在音频下载 URL。

下载、音频缓存、均衡器、音频分析/转发与 Automix 暂不支持 Apple Music。推荐、专辑/歌手详情和歌单写入不在本次范围。
